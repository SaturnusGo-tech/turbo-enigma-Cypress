...
class HomePage {
    ...
    checkImageVisibility() {
        ...
        cy.xpath(imageUrl).should('exist').should('be.visible').then($img => {
            if ($img.is(':visible')) {
                cy.log('Image is visible on the page');
            } else {
                // Прокрутка страницы до изображения, если оно не видимо
                cy.xpath(imageUrl).scrollIntoView().should('be.visible').then(() => {
     ...
     */
    openCart() {
        cy.get('a[href="/cart"]').first().click().then(() => {
            cy.log('Clicked on the first cart button');
        });
    }
}
